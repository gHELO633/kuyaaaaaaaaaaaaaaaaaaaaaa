...
Baggage Weight Input & Excess Fee Calculation using Recursion with IF/ELIF/ELSE
print("\n--- Baggage Information ---")
...
Function replacing the loop using only IF/ELIF/ELSE and recursion
def calculate_baggage(current_traveler, max_travelers, current_total_fee):
if current_traveler > max_travelers:
return current_total_fee
else:
weight = float(input(f"Enter baggage weight for Traveler {current_traveler} (kg): "))
if weight > 15:
excess = weight - 15
current_total_fee += excess * 100

return calculate_baggage(current_traveler + 1, max_travelers, current_total_fee)

total_baggage_fee = calculate_baggage(1, travelers, 0)
